...
### 2025.09.17 - 통계 시각화 개선
- ✅ **달력 형태 통계 뷰**: 기존 행 형태 일별 통계를 직관적인 달력 구조로 전면 개편
- ✅ **퍼센티지 시각화**: 전체 사용자 대비 식수 인원 비율을 프로그레스 바와 숫자로 표시
- ✅ **요일별 구분**: 각 날짜에 요일 표시 및 주말(토/일) 빨간색 테마 적용
- ✅ **호버 상세 정보**: 마우스 오버 시 해당 날짜의 식수 인원 및 이용자 명단 표시
- ✅ **Google Sheets API 최적화**: 빈 셀 처리 개선으로 월말까지 정확한 데이터 표시
...
- ✅ 관리자 대시보드
- ✅ 통계 분석 페이지 (달력 형태 일별 뷰, 주별 데이터)
- ✅ 달력 형태 통계 시각화 (퍼센티지, 요일, 주말 구분)
...
- `/admin` - 관리자 대시보드
- `/stats` - 통계 분석 페이지
- `/statistics` - 확장된 통계 페이지 (달력 형태 일별 뷰, 주별 통계)
...
1. **통계 조회**: `/statistics` 페이지에서 달력 형태 일별 통계 및 주별 통계를 확인
   - 달력 뷰에서 각 날짜별 식수 인원과 퍼센티지를 한눈에 확인
   - 주말(토/일)은 빨간색으로 구분 표시
   - 마우스 오버 시 해당 날짜의 상세 정보 및 이용자 명단 확인
